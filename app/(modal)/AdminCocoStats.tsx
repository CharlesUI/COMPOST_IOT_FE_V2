...
const AdminCocoStats = () => {
  const { admin, token, abortController } = useAdmin();
  ...
  const fetchChartData = useCallback(
    async (timeFrame: string, parameter: string, dataType: "compost" | "teg") => {
      ...
      try {
        ...
        if (!response.ok) {
          const message = `Coco chart data fetch failed: ${response.status}`;
        ...
        <View className="bg-[#1E1E1E] rounded-3xl overflow-hidden shadow-lg mb-6">
          ...
          >
            <View className="flex-row items-center">
              ...
              <View className="bg-[#10B04B]/20 w-10 h-10 rounded-full items-center justify-center mr-3">
                <FontAwesome5 name="seedling" size={18} color="#10B04B" />
              </View>
            ...
            {/* Removed the close button as the back button is added */}
          </LinearGradient>
          ...
          <View className="p-4">
            ...
            <View className="bg-[#2F2C2C] rounded-xl overflow-hidden shadow-lg mb-4">
              {isLoading && !refreshing ? (
              ...
              ) : selectedDataType === "compost" && chartData?.compostContainerOne?.length! > 0 ? (
                <LineGraphDataVisual
                ...
                />
              ) : selectedDataType === "teg" && chartData?.tegOne?.length! > 0 ? (
                <LineGraphDataVisual
                  deviceTime={deviceTime}
                  chartData={chartData}
                  isLoading={isLoading}
