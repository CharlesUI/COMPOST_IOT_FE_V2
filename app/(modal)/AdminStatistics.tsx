...
const AdminStatistics = () => {
  ...
  const sendNotification = async (message: string) => {
    if (!deviceIdentifier || notificationSent) return;

    try {
      const response = await fetch(
        `${API_URL_BASE}/admin/device/${deviceIdentifier}/notify`,
        {
          method: "POST",
          headers: {
      ...
      console.log("sending notif to", deviceIdentifier);
    ...
    try {
      ...
      if (responseData.realTimeData && responseData.realTimeData.timestamp) {
        ...
        if (
          ...
          sendNotification(
            `Your device ${deviceIdentifier} hasn't sent data for over 30 minutes. Please check if everything is functioning properly.`
          );
  ...
  }, [deviceIdentifier, token, API_URL_BASE, notificationSent]);
